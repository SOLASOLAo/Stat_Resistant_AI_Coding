    ...
    public sealed class ForceTraceFrames
    {
        ...
        public void ObserveStatistics(object value, bool good, long now)
        {
            lock(gate)
            {
                ...
                if(!good) { DisconnectCore(); return; }
                // A rejected transport snapshot says nothing about the retained record.
                // It must not refresh the heartbeat or erase the last coherent frame.
                if(!TryStatistics(value,out next)) { CheckTimeoutCore(now); return; }
        ...
        public void ObserveTrace(object value,bool good,TracePosition expected,DateTimeOffset received)
        {
            ...
            {
                TraceSnapshot next;
                if(!good) {store.ClearTrace(expected);return;}
                if(!TryTrace(value,received,out next) || next.Position!=expected) return;
                StatisticsSnapshot current=store.Statistics;
                if(current!=null)
                {
                    // Statistics establish the boot identity. Session refreshes the
                    // retained item when a new boot arrives, including trace-first order.
                    if(next.BootId!=current.BootId) return;
                    if((next.Cycle!=current.Cycle || next.Acquisition!=current.Positions[(int)expected-1].Acquisition) &&
                       unchecked(next.Sequence-current.Sequence)>Int32.MaxValue) return;
                }
                ...
                if(previous!=null && previous.BootId==next.BootId && previous.Cycle==next.Cycle && previous.Acquisition==next.Acquisition)
                {
                    ...
                    if(advance==0 || advance>Int32.MaxValue || next.Samples.Count<previous.Samples.Count) return;
                    // v2 freezes the trace at its first terminal cause. Only the
                    // publication sequence changes afterward; retain the same snapshot.
                    if(previous.EndReason!=TraceEndReason.Running) return;
                }
                store.Accept(next);
            }
        }
        ...
        {
            result=null;uint[] w;
            if(!Words(value,30036,out w) || w[5]<1 || w[5]>3 || w[6]>5 || w[7]>10001 || w[8]>1 || !SamplePeriod(w[16])) return false;
            ...
            else if(w[3]==0 || w[4]==0 || !Parameters(w[12],w[13],w[14],w[15],w[16])) return false;
        ...
        private static bool SamplePeriod(uint period) {return period>=1 && period<=1000;}
        private static bool Parameters(uint threshold,uint limit,uint window,uint timeout,uint period)
        {return SamplePeriod(period) && Finite(Float(threshold)) && Float(threshold)>=0 && Finite(Float(limit)) && Float(limit)>0 && window>0 && window<=60000 && window<=period*10000 && timeout>window;}
        public static bool TryStatistics(object value,out StatisticsSnapshot result)
        {
            result=null;uint[] w;
            if(!Words(value,210,out w) || w[4]>3 || w[6]!=20 || !SamplePeriod(w[7])) return false;
            ...
            for(int n=0;n<3;n++)
            {
                ...
                if(w[b]!=(uint)n+1 || w[b+2]<1 || w[b+2]>5 || w[b+3]>10002 || w[b+4]>31 || w[b+22]!=20 ||
                   !Parameters(w[b+11],w[b+12],w[b+13],w[b+14],w[b+44]) || w[b+16]>w[b+17] || w[b+17]>w[b+15])return false;
