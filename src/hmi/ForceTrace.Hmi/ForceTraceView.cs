...
namespace ForceTrace.Hmi
{
    public sealed class ForceTraceView : SmartControl
    {
        ...
        private readonly Label status=new Label(),metrics=new Label(),traceTitle=new Label(),distributionTitle=new Label(),samplingNotes=new Label();
        ...
        public ForceTraceView()
        {
            ...
            foreach(string key in new[]{"Left","Middle","Right","Save","Saving","Saved","SaveFailed","NoData","ForceAxis","TimeAxis",
                ...
                "ConnectionLost","BindingRequired","Truncated","NoStatistics","SamplingRule","QualificationRule","SigmaRule","StatisticsRule"})AddText(key);
            ...
            chart.SetBounds(12,97,552,280);chart.DataMode=ChartDataMode.Overwrite;chart.AllowFreeze=false;
            ...
            density.SetBounds(582,97,350,280);Controls.Add(density);
            metrics.SetBounds(12,389,920,52);Controls.Add(metrics);
            status.SetBounds(12,445,920,22);status.AutoEllipsis=true;Controls.Add(status);
            samplingNotes.Name="SamplingNotes";samplingNotes.SetBounds(12,470,920,100);
            samplingNotes.Font=new Font("Microsoft YaHei UI",9);samplingNotes.Padding=new Padding(8);
            samplingNotes.BackColor=Color.FromArgb(247,249,250);Controls.Add(samplingNotes);
        ...
        private void ApplyTexts()
        {
            ...
            density.EmptyText=Caption("NoStatistics");density.ConcentrationText=Caption("ZeroSpread");
            ...
            string period=shownStats!=null?shownStats.NominalSampleMs.ToString(CultureInfo.CurrentCulture):displayed!=null?displayed.NominalSampleMs.ToString(CultureInfo.CurrentCulture):"—";
            string threshold=shownStats!=null?Number(shownStats.ThresholdN):displayed!=null?Number(displayed.ThresholdN):"—";
            string window=shownStats!=null?shownStats.WindowMs.ToString(CultureInfo.CurrentCulture):displayed!=null?displayed.WindowMs.ToString(CultureInfo.CurrentCulture):"—";
            string limit=shownStats!=null?Number(shownStats.SigmaLimitN):displayed!=null?Number(displayed.SigmaLimitN):"—";
            samplingNotes.Text=String.Format(CultureInfo.CurrentCulture,Caption("SamplingRule"),period)+Environment.NewLine
                +String.Format(CultureInfo.CurrentCulture,Caption("QualificationRule"),threshold,window,limit)+Environment.NewLine
                +Caption("SigmaRule")+Environment.NewLine+Caption("StatisticsRule");
        }
