...
Each entry identifies its exact object/framework version, local read-only
source path, commands, important parameters/feedback and integration lifecycle.
Framework entries additionally record verified Chain and coding conventions.
Unverified assumptions must be marked `pending`, not promoted to a reusable rule.
