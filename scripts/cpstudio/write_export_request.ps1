...
if (-not $PlcProject) {
    ...
    if ($configuredPlcProject) {
        ...
        if ($candidate) {
            ...
            # Keep the legacy Station010 convention in the request even when the
            # generated project is temporarily absent; the consumer will report it.
            $PlcProject = Join-Path $plcDirectory 'Stat010_V5.11_CtrlX_PLC.project'
        }
    }
}
