...
>
> **2026-09-14 10:55 PartCounter 移除已完成本地工程，现场更新待批准。** 用户要求删除原超时提示，并授权选择较快路径；未发现现成单条隐藏开关后，已通过 CpStudio 删除 Station 的 PartCounter add-on 和 APQ 的 PartCounter runtime 项，Save All 保存。原 `Station010/PartCounter` 文件夹保留；检查点 `data/checkpoints/partcounter-remove-20260914-102642/`。Fast export 清除了声明、AddAddOn 注册及参数，其他变化仅空行；OnCall 原样保留。原生 Symbol 编辑器删除四个失效 PartCounter 类型签名并取消旧变量选择。
>
> **最终新 F11：0 errors / 原 5 warnings。** Fast export 将九个电阻成员权限恢复为默认 ReadWrite，已用官方精确 Select 一次 PUT（HTTP 200）恢复九个 Read，Save job Done 后重新 F11；父结构、曲线帧 Read，最终 Symbol 无 PartCounter。五组 PlanOnly 均零操作；Burster 15 对象与 canonical 去除首尾空白后一致，旧 writer 的 SelectProgram 末尾换行限制仍在，未回写业务代码。HMI 单独 Export 为 0 errors / 原 2 warnings，config.xml 无 PartCounter；Gui 本次未变且隐藏项仍为 True，但 Export 删除了隐藏启动 SFC，已从已验证的 .2 staging 恢复并核对三张 SFC。以后不能只检查 Gui，必须同时检查隐藏文件实际存在。
>
> **本批未连接/下载/启停 PLC，未再部署 IPC，也未推送 GitHub。** 当前现场仍有原提示。之前授权只有本批 HMI 更新/重启；待用户批准停稳设备后的 PLC 下载及 HMI 同步。当前本地 PLC 还包含此前已编译但未下载的电阻每次上下限判定、换型重入及曲线发布改动，现场更新需一起验收。CpStudio 和唯一 PLE 保持打开，PLE 离线；本轮由 CpStudio 原生启动 PLE，MCP 仍无 owner，后续用现有官方 REST/原生界面，勿再启动第二实例。证据 `data/reports/partcounter-remove-20260914/`。
>
> **2026-09-14 后续：用户已批准现场 IPC 的 HMI 更新与重启，已完成。** 核对 APQ=192.168.0.50、HMI 路径 `C:\OpconApp\HMI`，更新前为 .1 且无隐藏启动项；285 文件已备份至 `data/checkpoints/hmi/ipc-192.168.0.50-before-20260914-100410/HMI/`，8 个关键文件备份核对通过。通过 CpStudio APQ→HMI→Deployment，仅选 HMI If Required，执行一次原生部署；最终 IPC .2 DLL、Gui、启动 SFC、两张业务页和双语文件共 8 项与本机一致，唯一隐藏启动项为 True。本次原生 Deployment 保留了已补齐的启动配置，无需再补写。
>
> **已确认现场 HMI 重启并显示电阻结果及中文菜单，尚未执行自动验收。** CIM 回读实际进程 `C:\OpconApp\HMI\OpCon.HMI.Modulo.exe`（PID 4636），启动时间为 IPC 时钟 10:07:43；IPC 时钟比本机慢约 82 秒，未改时钟。当前启动日志未见曲线/SmartForm 加载错误；“手动功能、测试功能、操作引导、工位指示灯、力曲线”和三位置电阻中文可见，原 PartCounter 超时提示仍在。用户手动完成 RDP 登录，AI 未打开曲线页、未触发测量/运动、未连接或下载 PLC、未操作 GitHub；实际 zh→en→zh、先完成左中右再开曲线页、第二轮及故障场景仍待现场验证。证据 `data/reports/hmi/force-trace-ipc-20260914/`。CpStudio/现场 HMI 保持打开，不安排后台任务；后续 Export/Smart 设计器仍可能移除本机隐藏启动项，按下述规则处理。
>
> **2026-09-14 本机曲线 .2 原生升级完成，现场部署待授权。** 开始时 CpStudio/PLE 均关闭，MCP 无 owner；沿用已批准的自有 Std 例外，备份后原生移除 .1、正常重启 CpStudio、安装 .2 HAD。Gui/Toolbox 各一份 .2 引用；实际曲线设计页打开、保存、关闭及重开成功，HMI 单独 Export **0 errors / 原 2 warnings**。已安装 DLL 的隐藏预加载/三位置晚开重开合成帧检查、48 项双语资源及 36 项动态文字检查通过。电阻 UserDefined 未变，VWN 只变修改时间；PLC/Symbol 仍与此前保存版本一致，本轮未启动 PLE 或重新编译 PLC。
>
> **启动配置的重要限制已实际复现：CpStudio 不仅 HMI Export，重新打开 Smart 设计器也会重建 Gui.config 并移除手工启动项。** 最后已关闭设计页、Unload Smart project，再补回并回读唯一 `BppForceTraceStartup`（loadOnStartup=True），对应 `Hmi/SmartForms/Bpp.ForceTrace/ForceTraceStartup.sfc`。最终部署必须在最后一次生成/设计器操作后使用 `data/reports/hmi/force-trace-upgrade-20260914/startup-overlay/`，或针对最新导出重新运行既有 New-ForceTraceStartupOverlay；启动 IPC HMI 前核对目标两文件及 .2 DLL。不能仅替换 DLL、也不能把本机接入当成现场修复。证据 `data/reports/hmi/force-trace-upgrade-20260914/local-upgrade.json`，备份 `data/checkpoints/hmi/force-trace-upgrade-20260914-093620/`。CpStudio 保留打开、Smart 工程已卸载；无实体连接/动作、PLC 下载、IPC 写入或 GitHub 推送。本轮未扩展封装体系，Burster 独立库仍待后续明确安排。
>
> **2026-09-11 封装收尾，按用户要求简化并停止。** 用户要带电脑回家，不继续扩大哈希/manifest/兼容矩阵，不安排后台任务。当前直接入口是 `components/station010/START-HERE.md`；可用本地候选目录 `data/components/station010-set-20260911-rc.4/`（188 文件），三核心包继续使用冻结 rc.2，ForceTrace DLL/HAD 0.1.0.2。含最新每次 Ω 上下限判定、独立电阻 OK/NOK 绑定、中文、曲线晚开与换型参考。rc.3 的 Windows PowerShell 5 contentId 校验确曾失败，定位为两个文档路径在 NLS/ICU 下排序不同；rc.4 仅对组合校验入口去掉再次排序，刚完成的构建及 PS7/WinPS5 接收校验通过，旧包均未覆盖。内部证据保留 `data/reports/components/station-set-20260911-180713/`，不把校验操作作为使用者必走流程。
>
> 四项包内 PLC 源码契约通过；冻结 .2 DLL 的原生隐藏启动/晚开/重开、121 点图/CSV、48 项资源及 36 项动态文字检查通过。23 项既有打包检查通过；此前 211 个冻结文件再次核对未变。没有新独立库编译/真机验证；Burster 独立消费仍未通过。现有 PLE 已在线，保持会话原状；本轮不连接/退出在线、下载/启停/FORCE、部署 IPC 或写 GitHub。CpStudio 模型与 TypeData 曾于 18:01 再保存，旧 F11 只关联仍相同的 PLC/Symbol 字节，不扩展到后续模型变化。**本轮成果已落盘，停止操作，无本轮遗留运行任务；下次按简短说明续接现场验收，勿自动重开封装工作。**
>
> **2026-09-11 HMI 判定来源澄清并核对。** 用户明确电阻结果界面 OK/NOK 不得使用 Kistler Unit 的输出。实际 UserDefined 三个 Status 的文字/颜色均绑定对应 `Station.HMIResistance._HmiResistance{Left|Middle|Right}Ok`，可见性由对应 Valid 控制；现有 PLE OnCall 回读确认来源是父链各位置的 `Resistance.Ok`，N090 回读为原始 Ω 电阻与本次上下限的显式比较，无 Kistler OK/NOK 参与。已满足该要求，本轮无需改 PLC/HMI 或再编译；Kistler 的力采集/力门禁仍保留。当前 Application 已由外部变为在线，工程 SHA 仍 `1762c81b21d37ee994568e3134bf7ecf3f023bcbc3b4cf3fd957a9a8b81c6f81`；本轮仅 GET，不执行连接/退出在线/下载/动作，也不把“在线”本身当作现场判定验收。
>
> **2026-09-11 最新：每次电阻测量的显式上下限判定已离线完成。** 用户将 TypeData 两限改为 Ω，导出 Label2 已核对；用户明确需要每次结束判范围。仅 N090 一个 PLC Action PUT/Save：保留力/命令诊断，首次成功完成时用原始 Ω 电阻与 N080 为本次命令复制的 ParCmd 限值比较（下限≤值≤上限，且 NOT OutOfLimit），锁存现有 Valid/Ok；左中右复用同一逻辑，单步停留不重判。标准 Unit/驱动、生成接口/SFC 图、父链及 HMI 映射未改。12 个实际表达式边界检查及力/Kistler/SFC/框架检查通过，41 个目标保存回读、最终 PlanOnly 0 操作。**本次新 F11（17:51:02）：0 errors / 原 5 warnings**（4×C0351、1×C0373）。Project Pack VALID，三个冻结组件源检查仍通过；未覆盖既有组件包。详见 `docs/reviews/station010-resistance-grading-20260911.md`，证据 `data/reports/plc/resistance-grading-20260911-174805/verification.json`。下方“单位/单次判定未闭环”为修改前历史。本批未下载/测量/部署 IPC/推送；实际活动配方 Ω 数值及现场超差/重复周期仍待验收，不把模板默认 0 当在线值。本次需求是各次判定，不扩展整件总判定或 NOK 停机策略。
>
> **2026-09-11 TypeData 电阻上下限只读核对。** 现有 PLE REST 回读 N080/N090、父链 N030/N060/N090/N999、StationUnit.OnCall 及驱动 ParseNumber/MeasCmd：每次 SINGLE_MEAS 前从 active TypeData 直接复制 UpperLimit/LowerLimit，完成后锁存标准 Unit 的 ResistOk，三位置各自保留并接入 HMI。原厂 Unit 文档区分 ResistOk（公差合格）与 OutOfLimit（超仪表量程），不能混用。**单位仍有待闭环问题**：当前 `TypeDataSetManagerL1.dat` 两限 Label2 为 mΩ（CP936），实际驱动统一返回 Ω，而 N080 没有换算；原厂公开说明未写明 Unit 内部是否换算/边界比较式，不能声称已经确认所有判定正确。父链 N999 只报告测量完成，没有三位置质量汇总/按 NOK 分支。下一步核对 Unit 单位契约及活动配方，再决定最小修正；本轮不改 PLC/配方、不连接/下载/触发测量。PLE 离线，profile 2.6.8，工程 SHA 前后均 `4fd4f0115b0441aecd301c12e9cb0dcfb47ea0e90bed30b689844c86d8cfc663`，与上一轮 `2e876c…` 已不同，不把旧包锁视为当前工程身份；本轮只更新交接/待办，无新编译或现场判定验证。
>
> **2026-09-11 最新：晚开曲线页修复候选、菜单中文与复用包完成。** 用户确认自动开始后才开页；旧 .1 在 View.OnLoad 才订阅，LEFT 当作初次基线，无历史样本可补。只读 IPC 的 DLL/视图/语言与本机 .1 一致，实际 MIDDLE 曲线及既有 CSV 可见。新 DLL **0.1.0.2** 增加每 Item 唯一 session 和隐藏 SmartForm 原生预加载，Frame v1 / PLC 不变。原厂 Gui.config 预加载、先录左中右后开页、销毁重开均通过合成帧检查；不能直接把原 SmartControl 的 loadOnStartup 改 True。**.2 仅候选，未安装/部署**；当前本机/IPC 仍 .1，旧版仍需先开页。
>
> **中文原生保存/导出完成。** 补手动功能、测试功能、操作引导（菜单+标题）、工位指示灯，保存用户“换型”，保留“主界面/自动/手动/回原位”。模型只变 6 个 zh_CN 单元、无增删行；中文导出更新 10 项、英文不变，VWN 仅修改时间。HMI Export **0 errors / 原 2 Burster 平台 warnings**；48 资源 + 36 动态提示检查通过。业务 SFC/9 个电阻绑定/PLC/Symbol 未变。证据 `data/reports/hmi/force-trace-startup-20260911/`；说明 `docs/hmi_force_trace_addon.md`、`docs/hmi_localization.md`。实际 OPC 启动时序、运行时语言切换、取消/断线/第二轮仍待现场验收。
>
> **BPP-REUSE-PACKAGING-20260911 本地交付完成，原生库阻点明确保留。** 复用原 Build-DeviceComponent，Burster/Kistler/ForceTrace 源码版本均 **0.1.0-rc.2**，HMI DLL/HAD **0.1.0.2**。manifest/lock 使用 Git 锚点+实际 dirty 源码逐文件 SHA/摘要，带接口/单位/状态/错误/依赖及安装/升级/回滚说明。当前离线 PLE 库表已只读刷新，Kistler Base 精确为 AtmoKistlerForceStrokeBase 1.2.1.0，ctrlX 平台重定向已记录。23 打包门禁、包内 Burster 18 项、Kistler 生命周期、ForceTrace 59 项及原厂启动晚开测试通过；另一组 Item/TextGroup 的原厂加载/121 点图表 CSV 也通过。三个重复 Build 均 UNCHANGED；旧 rc.1 的 27 文件 SHA 保持。
>
> **交付** `data/components/station010-set-20260911-rc.2.zip`（96 文件，195493 字节），ZIP SHA `DEE254985912CF585E745AEE744C16FB4250101D1BF22792D3E77B47F998D380`，set contentId `AE8FDBE99DAA4191862C3BA4CD4946E795B848C1020C2B5464F0573CAA0E7D5A`。逐 ZIP 项对照及包内 Verify.ps1/Windows PowerShell 5 校验通过；含自有代码/HAD、Text 生成器、页面/启动模板、PLC 发布器、本站 overlay/文本增量/组合锁，不含整站凭据、加密工程、原厂 DLL、失败 compiled-library。入口 `docs/station010-component-set.md`，报告 `data/reports/components/reuse-20260911/`。
>
> **Burster 正式 .library 尚未完成。** 上次原生库独立消费失败仍需修复；本轮不关闭/重启/接管当前 CpStudio Station010 PLE，也不启动第二个相同 profile writer，因此未重新试建。交付已校验源码候选和真实依赖/失败证据；待空闲独立库窗口修 namespace/ctrlX 重定向并 fresh library+consumer Build，正式 Company/固件亦待确认。没有新建/管理旁线自动任务。
>
> **本轮无 PLC 修改/下载、实体连接/启停/FORCE、IPC 写入或 Git 提交/推送。** PLC SHA `2e876c…a6da`、Symbol `9ffcab…c9d6` 保持，当前 PLE 离线并保留打开。下一步正常本地原生 .had 升级及授权后整体 HMI 部署验证；Changeover 本地 PLC 修复仍须另行下载复测。完整 PLC Export 原厂流缺陷仍未解决。以下 .1/rc.1 是历史安装/冻结状态，由本節候选记录补充。
>
> **2026-09-11 最新：HMI 双语本地接入及 Changeover 重入修复已保存，现场待验收。** 先完成 HMI-ZH-EN-20260911，再处理 CHANGEOVER-REENTRY-20260911。CpStudio 新增 6 个电阻原生 Text、补齐页面菜单中英文；数值标题/未完成提示绑定 Station TextGroup，OK/NOK 复用标准资源。保留原 5 控件、944×572 画布、9 个数值/业务绑定和资源，UserDefined 新 SHA `7ab00c…89bc`。HMI 单独 Export **0 errors / 原 2 warnings**，48 项导出文本与 36 项离线动态提示检查通过，英文宽度合适；原生运行时语言管理器的真实切换仍待现场验收。详见 `docs/hmi_localization.md`。
>
> **换型第二次卡住的证据与修复。** 在线 N010 的 StationViewRequest=`ChgOvGuidanceView`、StationDialogRequest 空、CurrentStep=INIT、返回 RUNNING；旧 N010 只接受空视图，而正常/取消/错误路径均未释放该请求。正常 Logout 后确认离线，备份哈希一致，仅 PUT N010、_reset、N999 三个实现：复用本链视图请求，正常结束/取消/错误清理本链视图和文件对话框请求，其他页面请求保持。Save/回读通过，SFC 图、全部声明及其余 20 子对象未变；新的 F11 **0 errors / 原 5 warnings**（4×C0351、1×C0373），最终 PlanOnly **0 操作**，10 项离线请求生命周期检查通过。未下载，因此现场旧逻辑尚未修复；详见 `docs/reviews/station010-changeover-reentry-20260911.md`。
>
> **当前工程边界。** 最新 `.project` SHA `2e876c…a6da`，Symbol SHA `9ffcab…c9d6` 保持；所有证据在 `data/reports/hmi/hmi-zh-en-changeover-20260911/`。PLE 已 Logout 并保留打开，未停止 runtime、未写在线值、未部署 IPC、未提交/推送 GitHub。本地新代码需要另行授权下载后验证连续两次换型及取消重试。以下曲线集成记录仍有效，但其中早先的 UserDefined 固定中文/旧哈希已由本节取代。
>
> **原生注册及设计器已通过。** 用户批准的有限 Std 例外已执行：只管理自有 `Addons/Bpp.ForceTrace` 和 HMI 启动 probing，其余 Std/vendor DLL 未改。直接升级曾留下旧引用并报反射程序集加载错误；已通过原生移除、正常退出/重开清除旧引用及文件占用，再安装 **0.1.0.1**。Gui/Toolbox 各一个新版引用，安装 DLL 哈希匹配测试版本，实际曲线设计器已打开、保存、关闭并重开，无错误；空记录为空图。无需重新导入旧包，TEMP/Bpp.ForceTrace.had 当前为 .1。安装包 `data/reports/hmi/force-trace-native-20260911-133634-189/Bpp.ForceTrace.had`。
>
> **HMI 单独 Export 完成；完整 PLC Export 缺陷仍存在。** CpStudio HMI Export 为 0 errors / 2 条原有 Burster 平台 warnings；VWN 新数组为 VT_UI4/Cnt=14，9 个电阻项保留。去掉修改时间、数组 Item/Text 及新页面菜单 Text 后，整个 VWN 与此前一致，PLC 工程/Symbol 哈希未变化。曲线 Symbol 精确 Select 曾超时，等待内部完成后保存、编译与 XML 核对，只新增 Read 数组节点及类型，未重放请求。完整 PLC Export 的符号流截断已在原厂 DLL 3.3.2.1183 的隔离延迟流中复现，未修原厂 DLL；不要继续盲目 Export/重启 PLE。详情 `docs/reviews/cpstudio-symbol-stream-20260911.md`。
>
> **验证与使用边界。** 59 项记录/帧测试，实际安装 DLL + 本站 SFC 的帧→原厂图表→CSV、121 点坐标/CSV/空点检查通过；工程框架、力联锁、Kistler 生命周期和 SFC 完成契约通过。HMI 启动后先打开 ForceTrace 页，再开始要记录的周期（loadOnStartup=False、KeepAlive=True）；初次/重连订阅不补录旧采集。实际 OPC Item 订阅质量、现场 t=0/三位置/取消/断线/重复周期、原生运行时语言切换及 IPC CSV 权限仍待验收。电阻双语资源现已接入，离线渲染不等于 IPC 运行时切换。Kistler 冻结 0.1.0-rc.1 的 Check 因本站 N050/OnChainFinish 新增记录钩子报 SOURCE_DRIFT，已审阅；冻结包/版本锁未改，新版待现场验证后单独发布。
>
> **工程计划检查**：本次新增 3 条换型实现 ownership/hook 后重新 Build/Check 为 VALID，只改变 sources 清单及 contentId（`5c99fe…aea6d`），SFC 计划和 I/O 完全相同；框架检查为 38 core files / 85 ownership records / 76 PLC sources。额外旧 Test-Station010ProjectPack.ps1 仍有已知 23/27 步基线差异，本轮未修改或重跑；不能称全部仓库测试通过。新证据 `hmi-zh-en-changeover-20260911/project-pack-scope.audit.json`。
>
> **当前授权仅继续本地实现/保存/导出/离线验证；本批未下载、未触发测量或运动、未部署 IPC、未提交/推送 GitHub。** 不沿用下方 09-10 历史现场授权触发新动作。现有 CpStudio/唯一 PLE 保留，UserDefined 设计页已保存、PLE 已 Logout。接入说明 `docs/hmi_force_trace_addon.md`；早先曲线配置、PLC、Symbol、导出及安装审计在 `data/reports/hmi/force-trace-registration-20260911/`，真实帧离线加载检查在 `data/reports/hmi/force-trace-installed-20260911-135722/native-check.json`。

> **2026-09-10 实时力曲线/事件接入范围已明确（最新）**：用户选择 PLC 实时力，不取仪表内部完整曲线；CSV 计划保存在 IPC .50 本地，按左/中/右分别记录。已核实本机 CpStudio 5.11 标准 Kistler Chart 使用 `Mod_Chart`/Writer/单元素 X/Y 趋势数组，可选 WriterHistory；ScopeView 录制依赖 TwinCAT Scope Server、输出 SVD，不能当作现成 ctrlX CSV。未发现已文档化的 Mod_Chart CSV 按钮，需先验证受支持的 HMI 适配接口；未创建自研 logger 或改生成视图。规格 `specs/hmi/plc_force_curve.yaml`，人工配置与建议 Wp100 20–28 事件见 `docs/reviews/station010-force-curve-events-20260910.md`。已核对导出表的现有号，新增事件仍是提案、未在 CpStudio 创建、未写 PLC 触发。

> **2026-09-10 Burster 原生库试构建未通过（取代下方“从未构建”的时态）**：隔离 `.library` 的 15 对象回读与原源码一致，产生 `.compiled-library` 并做临时安装/独立消费验证；新消费 Build 因库引用限定名/平台依赖失败，最终摘要 500 errors / 2 warnings（达到错误上限、细节有界），绝不是 Station010 编译结果。测试工程已保存关闭，本轮临时 `BppBurster2316 0.1.0.1` 已通过官方 API 卸载，库仓库查询为空；恢复文件留在 `data/component-native/`，禁止安装失败候选。两个已冻结源码包仍有效，不覆盖同版本。详见 `docs/reviews/burster-native-library-20260910.md`。本轮未改 Station010/ST/Std、未下载/动作、未新增 GitHub 推送。

> **2026-09-10 设备组件化第一阶段（本地，未新增推送）**：用户同意将Burster自研驱动和Kistler标准驱动集成分别版本化。已建立`components/README.md`、两个`component.json`/接入说明、`config/component-versions.json`源码选用锁及纯离线`Build-DeviceComponent.ps1`。两组件版本均为`0.1.0-rc.1`，代码固定到已推送的`d37918a4863bdaaf7937df5daa3b948ef124e22d`；保留原POU名及唯一源码，不改本站驱动、SFC、绑定或仪表。Burster标为source-driver且仅一个核心driver文件，本站选择器/绑定/全局变量是reference；Kistler标为standard-driver-integration-kit，不是新通信库。输出为`data/components/burster2316-0.1.0-rc.1/`（14文件）和`data/components/kistler5867c-0.1.0-rc.1/`（13文件），重复Build均UNCHANGED。19项打包检查、包内Burster18项离线协议测试（执行时使用`python -B`避免生成缓存）、包内Kistler契约/状态模型、原工程框架/Burster契约及Project Pack Check通过。所有源码文件仍与基线一致；未访问PLE/PLC、未编译/下载、未创建标签或Release。**正式.library/.compiled-library尚未构建**：依赖精确版本/固件及Company部分待核对，后续需在不占用Station010的离线库会话中生成并编译，再在隔离消费工程验收，不能冒称已安装库。版本锁只控制源码打包，不是在线PLC库盘点。一轮自动/0号Active经验和质量/重复/故障恢复待验收边界已纳入组件说明。
