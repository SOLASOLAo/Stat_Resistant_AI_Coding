...
- [x] **2026-09-14 PartCounter 本地移除**：CpStudio 删除模型 add-on 和 APQ runtime 项，Save All、Fast export、原生清除四项失效签名和变量；九个电阻成员恢复 Read 后新 F11 0 errors / 原 5 warnings。五组 PlanOnly 零操作，Burster 15 对象内容一致。HMI 单独 Export 0 errors / 原 2 warnings，隐藏启动 SFC 已恢复，三张 SFC/.2 DLL 与绑定核对通过；保留原 PartCounter 文件夹及工程检查点。证据 `data/reports/partcounter-remove-20260914/`。
- [ ] **PartCounter 现场消警**：等待用户批准停稳设备后下载当前 PLC 版本并同步 HMI；本批未更新现场。下载也包含此前未下载的电阻判定、换型重入和曲线发布改动，随后核对提示消失及相关功能；不得把离线移除写成现场已消警。

- [x] **2026-09-14 本机 ForceTrace .2 原生升级**：正常移除/重启/注册，引用唯一；曲线设计器打开/保存/重开通过，HMI 单独 Export 0 errors / 原 2 warnings；已安装 DLL 的晚开/重开与双语离线检查通过。最终本地隐藏启动项已接回，PLC/电阻界面未改。
- [x] **2026-09-14 HMI .2 现场部署与重启**：用户明确批准后，备份 IPC HMI 285 文件，通过 CpStudio 原生部署仅更新 APQ HMI；目标 .2 DLL、唯一隐藏启动项及业务/语言文件核对通过，实际 HMI 新进程与电阻结果/中文菜单已确认。无 PLC 下载或测量动作，证据 `data/reports/hmi/force-trace-ipc-20260914/`。
- [ ] **HMI .2 自动场景验收**：保持 HMI 运行，先不开曲线页，完成左中右后再打开；再验第二轮、重开、断线及实际 zh→en→zh。原 PartCounter 超时提示仍在，本批未处理。后续 HMI Export/Smart 设计器会重建 Gui.config，仍须最后接回 startup-overlay 并核对 IPC；当前不能把已部署当作晚开曲线现场验收通过。

- [x] **按“简化、马上收尾”保存并停止（2026-09-11）**：直接使用本地 `data/components/station010-set-20260911-rc.4/`，入口 `components/station010/START-HERE.md`。三核心 rc.2 + ForceTrace .2 + 最新电阻/中文/换型接入层已保存；rc.3 排序校验失败如实保留，rc.4 小幅修正后构建及 PS7/WinPS5 接收检查通过。无下载、IPC 部署、GitHub 写入、后台续做。哈希/清单仅内部记录，不再扩展为使用前提。
- [ ] **下次按需继续实际验收**：独立 Burster 库消费仍未通过；HMI .2 及最新电阻/换型现场验收仍待。不要自动重启封装、扩大测试矩阵或清理旧包；电脑带走前保持 CpStudio/PLE 原状。

- [x] **电阻 HMI OK/NOK 来源核对（2026-09-11）**：按用户明确要求，三位置 Status 均为 `HMIResistance.*Ok ← Result.{位置}.Resistance.Ok ← N090 电阻上下限比较`，不使用 Kistler Unit.OutCmd.OK/Nok。本轮只读，无额外 PLC/HMI 修改；保留原力监控门禁。

- [x] **每次电阻测量显式判定（2026-09-11）**：用户改后的 TypeData 两限单位已核对为 Ω；N090 首次完成时明确比较本次 ParCmd 下限≤原始Ω电阻≤上限，含边界且要求无量程越界，锁存现有 Valid/Ok。三位置均覆盖；PLC 仅 1 个 Action 修改、41 目标保存回读、最终 0 操作 PlanOnly。12 边界用例和相关回归通过，新 F11 0 errors / 原 5 warnings。规格/工程计划已同步，冻结包不变；详见 resistance-grading review。
- [ ] **电阻判定现场验收**：本批尚未下载。授权部署后核对活动配方实际 Ω 数值，验证左/中/右各次合格、超差、边界及重复周期；通讯/力故障不得显示有效合格结果。本次不新增整件汇总、NOK 停机或自动重测。

## 2026-09-11 最新：晚开曲线与组件交付

- [x] **ForceTrace .2 后台采集本地候选**：旧 .1 到开页才订阅，用户确认自动开始后开页；新 session + 隐藏 SmartForm 经原厂预加载/晚开/重开合成帧验证，59 核心检查与 121 点图/CSV 保持通过；不补造旧 LEFT 数据。
- [x] **中文遗漏补齐**：原生保存 6 个 zh_CN 单元（含用户换型），HMI 单独 Export 更新 10 处文字、0 errors / 原 2 warnings，英文/VWN 业务配置/PLC/Symbol/9 电阻绑定不变。
- [x] **BPP-REUSE-PACKAGING-20260911 可复用本地包**：三组件源码 rc.2、HMI 原生 .2，manifest/lock/实际 dirty 快照与包 SHA、依赖/接口/迁移/回滚齐全；23 打包检查、包内协议/生命周期/曲线/启动及 WinPS5 校验通过。`data/components/station010-set-20260911-rc.2.zip` 96 文件，旧 rc.1 的 27 文件未改。
- [x] **原生 HMI .2 正常本地升级**：2026-09-14 已完成；现场部署/验收见最上方待办。只换 DLL 不足，最终启动配置也必须带上。
- [ ] **Burster 原生独立库/消费 Build**：保留当前 CpStudio/唯一 PLE，未重新试建；待空闲离线窗口修 qualified-only/ctrlX 平台重定向，再 fresh library+consumer Build。失败二进制未交付，正式 Company/固件仍待。
- [ ] **已有 Changeover PLC 修复现场复测**：当前本地版本未下载；另行授权后连续两次及取消重试。下方旧 rc.1 SOURCE_DRIFT/需先开页仅适用于历史版本。

- [x] **HMI-ZH-EN-20260911 本地接入**：新增 6 个电阻原生 Text、补齐页面菜单，名称/未完成提示改为 TextGroup 引用，OK/NOK 复用标准资源；原数值/业务绑定、坐标与资源不变。CpStudio 保存、HMI 单独 Export、重开通过；48 项导出文本、36 项离线动态提示及英文宽度检查通过。UserDefined 新 SHA `7ab00c…89bc`，详见 `docs/hmi_localization.md`。
- [ ] **HMI 原生运行时语言切换**：独立语言管理器检查未能初始化，离线翻译注入不算真实运行时验收。另行授权 IPC 部署后实测 zh→en→zh、未完成/OK/NOK、曲线运行/结束/取消/断线/保存提示和英文排版。
- [x] **CHANGEOVER-REENTRY-20260911 离线修复**：先保留在线 N010 RUNNING、视图请求残留/对话框空的证据，再正常 Logout。仅更新 N010、_reset、N999 三个实现并保存；图/声明/其他 20 子对象不变，10 项生命周期契约检查通过，新 F11 **0 errors / 原 5 warnings**，最终 PlanOnly 0 操作。详见 `docs/reviews/station010-changeover-reentry-20260911.md`。
- [ ] **换型现场复测**：另行授权下载后连续完成两次 TypeData 选择，并取消文件选择、取消/错误退出后重试。当前现场仍运行旧程序；本轮没有下载、在线修改、写值或停止 runtime。

- [x] **2026-09-11 电阻 Read 已完成**。原生编辑器 + 官方精确 Symbol Select 完成父结构及 9 成员 Read；该次 XML 只有这 10 个 access 属性变化。新增曲线阶段 F11 为 0 errors / 原 5 warnings，五个 PlanOnly 0 操作，Burster 15 对象内容匹配，该阶段 UserDefined SHA 保持（随后双语接入更新为上方新哈希）。完整 PLC Export 原厂流缺陷仍未恢复，Fast export 与 HMI 单独 Export 已完成。
- [x] **曲线原生扩展 0.1.0.1 已注册**：`.had` 仅含自有 DLL 与 AddonDesc.xml；按已批准有限 Std 例外通过原生 HMI add-ons 安装。原生移除旧版、正常重启清理旧引用后重装，Gui/Toolbox 各一个新版引用，安装 DLL 哈希匹配测试版本；实际设计器打开、保存、关闭和重开通过，空记录无演示曲线。证据 `data/reports/hmi/force-trace-registration-20260911/native-addon-final.audit.json`。
- [x] **曲线 HMI 单独 Export 完成**：Station.HMIForceFrame 生成 VT_UI4/Cnt=14 数组项，原 9 个电阻项保持；除时间、新数组 Item/Text 和新菜单 Text 外 VWN 全文相同，PLC 工程及 Symbol 哈希不变。CpStudio 0 errors / 2 条原有 Burster 平台 warnings。

- [x] **三位置显示接口已导出**：实际位置为 `Station.HMIResistance : HMIResistanceStruct`，9 个成员（3 REAL + 6 BOOL）已进入 Symbol/VWN，不再要求 Wp100 重建接口。
- [x] **PLC 电阻显示赋值已离线接入（2026-09-11）**：只增加 StationUnit.OnCall 显示片段，父链保留结果换算 mΩ，Valid/Ok 门控，无效值清零；1 PUT/Save/回读、原 OnCall 与生成声明精确保留，最终 PlanOnly 为 0 写入。本次新 F11 0 errors / 5 warnings（4×C0351、1×C0373），框架及事务检查通过；未下载/推送。
- [x] **UserDefined 三组控件自动接入（2026-09-11）**：备份后替换两个试放副本，增加三组数值/判定/未完成控件；保留原有 5 控件、944×572 画布、提示栏及图片资源。9 成员类型/路径、原厂加载器 0 错误、CpStudio 重开通过；三次 Export 尝试后 SFC SHA 保持不变。证据 `data/reports/hmi/resistance-view-20260911/`。
- [ ] **完整 PLC Export 原厂缺陷**：PLE 符号 REST 响应截断，独立相同请求及隔离延迟流复现；重开工程和重启唯一 PLE 均未解决。电阻父结构/9 成员及曲线数组的 Read 已核实，Fast export、HMI 单独 Export 和设计器保存重开已完成。仍需原厂修复或受支持的完整导出恢复途径，不再盲目重试 Export/重启；见 `docs/reviews/cpstudio-symbol-stream-20260911.md`。
- [ ] **显示现场验收**：每轮清零、左中右独立锁存、取消与 mΩ 换算；原生通讯质量/断线缓存单独验证，Valid 不等于通讯正常。新画面已离线创建/加载，尚未验证运行时数据和状态切换，未部署 IPC。

- [x] **PLC 实时力控件核实/范围确认**：用户确认 PLC 实时力、IPC .50 本地 CSV、分左/中/右。标准 Kistler 已有 Mod_Chart；ScopeView 是 TwinCAT Scope/SVD 路线，不当作现成 ctrlX CSV。见 `specs/hmi/plc_force_curve.yaml` 与 `docs/reviews/station010-force-curve-events-20260910.md`。
- [x] **实时曲线/CSV 与双语文本离线验证**：`src/hmi/Bpp.ForceTrace/` 使用原厂 Mod_Chart 和同一快照保存 CSV；28 项记录/CSV + 31 项帧检查通过，实际安装 DLL + 本站 SFC 的帧→图表→CSV、121 合成点坐标及空点检查通过，SFCLoader 0 错误。18 个 CpStudio 原生中英文 Text 已保存并生成；未把离线翻译注入视为运行时切换验收。
- [x] **实时曲线本地接入完成**：CpStudio 配置 Station/ForceTrace 与 Read 数组；PLC 发布器、周期/位置/结束/首错钩子已保存，HMI 只读 Item 绑定为 `Ch1.L1.Station.HMIForceFrame`。目标发布间隔 100 ms，时间取 PLC 毫秒计数，五组最终 PlanOnly 均 0 操作。未新增仪表命令、未改变原有力/运动联锁，详见 `docs/hmi_force_trace_addon.md`。
- [ ] **实时曲线现场验收**：HMI 启动后先打开 ForceTrace 页再开始待记录周期（loadOnStartup=False、KeepAlive=True）；首次/重连订阅不补录旧采集。核对真实 Item 质量和刷新、下压前 t=0、左中右与轮次、采集结束/取消/首错、断线/重连、运行时语言切换及 IPC CSV 权限。本批未下载、未部署 IPC。
- [ ] **Kistler 组件版本后续发布**：冻结 0.1.0-rc.1 的 Check 因本站 N050/OnChainFinish 新增记录钩子报 SOURCE_DRIFT，已审阅；旧包和版本锁保持不变，现场验证后另行更新版本，不覆盖冻结包。见 `components/README.md`。
- [x] **工程计划刷新**：Project Pack Build/Check 为 VALID，仅 ownership/hooks 清单哈希及 contentId 更新，SFC 计划与 I/O 未变。额外旧测试 `Test-Station010ProjectPack.ps1` 的步骤基线仍为 23 步，而 HEAD 规范已有 27 步，本批未改这两份文件；见 `project-pack-final.audit.json`。
- [ ] **既有 Project Pack 测试基线同步**：按已审阅 SFC 汇合步核对并更新旧测试的步骤/并行断言，另核对其历史 ASC 固定哈希；保留当前流程和 I/O，不能仅为了测试通过替换现场基线。
- [ ] **CpStudio 专用事件补充**：已核对现有 Wp100 1–5、12，建议 20–28 用于参数、力等待/下降、采力状态、压缸位置、Kistler准备、Burster选程/测量/清理。当前仅提案；人工添加并导出后才接精确触发、首错锁存/复位和 HMI 文本，避免旧 5 与新事件重复报警。CSV 文件错误先用本地提示。

- [x] **设备组件源码候选0.1.0-rc.1**：Burster驱动与Kistler集成包独立清单/文档/源码版本锁、纯离线Check/Build已完成；19项打包检查、Burster包内18项及Kistler包内测试通过，两包重复Build内容不变。沿用唯一canonical源码，无PLC源码/IDE/设备改动，无新Git标签/推送；见`components/README.md`。
- [ ] **Burster正式PLC安装库**：已试建15对象 `.library` 并临时安装/消费验证，但新 Build 依赖访问/解析失败；文件存在不等于合格库。试验库已卸载，消费工程已保存关闭，Station010 未动。先在独立库工程修正 qualified-only/ctrlX 平台解析并 fresh 消费 Build，补固件/正式 Company，才考虑现场替换；当前有效交付仍为源码包。见 `docs/reviews/burster-native-library-20260910.md`。
- [ ] **组件现场验收/正式版本发布**：结果数值/单位/判定、重复运行、取消、断线/重启恢复及Kistler力复位/故障保持仍待验收；版本锁目前是源码锁。通过后记录实际依赖与设备设置、建立组件标签/Release；不重新启用原Burster双连接方案，不把MP000 Active经验推广为全部固件要求。
