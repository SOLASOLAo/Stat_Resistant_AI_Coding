...
- Named Pipe v1 requires a caller-supplied trusted Broker PID and verifies the actual Pipe server PID plus current interactive Windows session. P1.2a contains no Broker and never starts PLE/MCP/Broker; no-session execution produces an immutable `BLOCKED_SESSION_UNAVAILABLE` result. `apply_change_set_and_build` remains blocked.
