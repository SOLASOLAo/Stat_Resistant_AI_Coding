...
- SFC/Symbol 修复后仍出现的 501 条 AddOn/BinIo 缺失级联，最终隔离为工程同目录的损坏 `Stat010_V5.precompilecache`。关闭正式工程后只把该缓存移到 `%LOCALAPPDATA%\Temp\Stat010_V5.precompilecache.stale-20260820`，重开后由 PLE 自动生成新缓存；连续两次真实 Clean Build 均为 **0 errors / 7 warnings**。`MainTask.core_binding` 已恢复 CpStudio 基线值 `-2`，复编仍通过，证明它不是根因。
